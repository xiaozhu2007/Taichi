FROM debian
COPY root /
RUN dpkg --add-architecture i386 ; apt update ; apt install wget wine xdotool dbus-x11 curl xfce4 xfce4-terminal firefox-esr busybox fonts-wqy-zenhei npm locales pulseaudio -y; \
    wget https://github.com/kasmtech/KasmVNC/releases/download/v1.3.1/kasmvncserver_bookworm_1.3.1_amd64.deb -O /a/kasm.deb ; \
    cd /a/ ; apt install -y ./kasm.deb; cd ~ ; \
